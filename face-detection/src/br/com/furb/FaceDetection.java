...
public class FaceDetection {
	...
	public static void main(String[] args) {
		...
		String filename = "D:\\dataset\\orl\\avengers.jpg";
		Mat detected = detectFaces(filename, classifier);
		Imgcodecs.imwrite("D:\\dataset\\orl\\avengers_detected.jpg", detected);
	...
	private static Mat detectFaces(String filename, CascadeClassifier classifier) {
		...
		classifier.detectMultiScale(grayImg, objects, 1.1, 1);
		
		objects.toList().forEach(rect -> {
			Scalar red = new Scalar(0, 0, 255);
			//Rect faceRect = new Rect(rect.tl(), rect.br());
			Imgproc.rectangle(img, rect/*faceRect*/, red, 3);
		});
